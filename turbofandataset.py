...
class Turbofandataset(Dataset):
    def __init__(self, mode='train', dataset=None, rul_result=None):
        # REMIND: using numpy's function to load dataset
        #           and delete unnecessary sensor columns
        ...
        # REMIND: following only for debugging issue
        ...
        if self.mode == 'train':
            for i in range(1, self.sample_num + 1):
                # REMIND: np.where will search in given data and return a tuple containing where the target exist
                cur_engine_index = np.where(self.data[:, 0] == i)

                # transfer tuple to ndarray
                cur_engine_index = cur_engine_index[0]

                # single engine data
                data_temp = self.data[cur_engine_index, :]
                for j in range(len(data_temp) - self.window_size + 1):
                    self.x.append(data_temp[j: j+self.window_size, 2:])
                    # REMIND: RUL data of a window will be set to the lowest one
                    rul = len(cur_engine_index) - self.window_size - j
        ...
        if self.mode == 'test':
            for i in range(1, self.sample_num + 1):
                cur_engine_index = np.where(self.data[:, 0] == i)[0]
                data_temp = self.data[cur_engine_index, :]
                '''
    ...
    def fea_extract(data):
        ...
        for i in range(data.shape[1]):
            ...
            # REMIND: np.polyfit will return a vector of coefficients that minimizes the squared error
